...
假设我们在数据库year2019、数据表table02，有学生的姓名和学校数据：
...
```PHP
//对应各个字段的数据，若不修改则留空
$test_array=array("字段名1","字段名2","字段名3","字段名4",...); 
```
...
利用文件包含的 **order()** 方法，我们可以快速根据其中一列，对数据表进行排序。
...
```PHP
$TEST->xy(m,n);  //返回表中坐标(m,n)的数据
```

2. **row()** 方法

```PHP
$TEST->row(m);  //返回表中第m行的数据的数组
```

3. **line()** 方法

```PHP
$TEST->line(n);  //返回表中第n列的数据的数组
```
